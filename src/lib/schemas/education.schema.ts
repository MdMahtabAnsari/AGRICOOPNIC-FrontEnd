...
export const educationSchemaOptional = z.object({
  ...
  institution: z.string().min(3, 'Institution name is required').max(100, 'Institution name must be less than 100 characters').optional(),
  boardOrUniversity: z.string().min(3, 'Board or University is required').max(100, 'Board or University must be less than 100 characters').optional(),
  subjectOrSpecialization: z.string().min(3, 'Subject or Specialization is required').max(100, 'Subject or Specialization must be less than 100 characters').optional(),
  marksType: z.enum(['PERCENTAGE', 'CGPA'], {
    error: 'Invalid marks type'
  }).optional(),
  marks: z.string()
    .min(1, 'Marks is required')
    .max(5, 'Marks must be less than 5 characters')
    .regex(/^\d+(\.\d+)?$/, 'Marks must be a valid number').optional(),
  yearOfPassing: z.string()
    .min(4, 'Year of passing is required')
    .max(4, 'Year of passing must be 4 digits')
    .regex(/^\d{4}$/, 'Year of passing must be a valid year').optional(),
}).refine(
  (data) =>
    !(data.marksType === 'CGPA' && data.marks && parseFloat(data.marks) > 10) &&
    !(data.marksType === 'PERCENTAGE' && data.marks && parseFloat(data.marks) > 100),
  {
    message: 'Marks value is invalid', // <-- Use a static string here
    path: ['marks'],
  }
